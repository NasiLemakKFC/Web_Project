...
if ($store_id) {
   ...
   $sql = "SELECT SUM(io.Total_Price) AS total
    ...
    $metrics['clean_profit'] = number_format($total_profit * 0.9, 2); // 10% fee
...
if (isset($_POST['action'])) {
    ...
    elseif ($_POST['action'] === 'cancel') {
        $sql = "DELETE FROM item_order WHERE Order_ID = ? AND Item_ID = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ii", $order_id, $item_id);
        $stmt->execute();
        $_SESSION['success_message'] = "Order canceled!";
    }
...
$sql = "SELECT o.Order_ID, o.Order_Date, i.Item_ID, i.Name AS item_name, i.Picture, 
       ...
       CASE WHEN r.Review_ID IS NOT NULL THEN 1 ELSE 0 END AS HasReview
        ...
        LEFT JOIN review r ON r.Item_ID = i.Item_ID 
                        ...
                        AND r.Order_ID = o.Order_ID  -- ✅ Match this too
        WHERE o.User_ID = ?
            ...
            <div class="tab-content" id="products">
                ...
                <div class="dashboard-grid">
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-label">Clean Profit</div>
                    </div>
                ...
                <?php else: ?>
                    ...
                    <div class="order-card">
                        ...
                        <div class="order-details">
                            ...
                            <div class="order-actions">
                            <a class="action-btn review-btn" href="edit_product.php?id=<?= $item['Item_ID'] ?>">Edit</a>
                            <a class="action-btn cancel" href="delete_product.php?id=<?= $item['Item_ID'] ?>" onclick="return confirm('Delete this item?')">Delete</a>
                            </div>
                        </div>
                        </div>
                    </div>
